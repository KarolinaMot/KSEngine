...
#include <old/DeferredRenderer.hpp>

DeferredRenderer::DeferredRenderer(DXDevice& device, DXShaderCompiler& shader_compiler, glm::uvec2 screen_size)
{
    ...
    // Pipeline Setup
    {
        ...
        auto deferred_ps = shader_compiler
                               .CompileFromPath("assets/shaders/Deferred.hlsl", DXShader::Type::PIXEL, L"mainPS")
                               .value();
        ...
        graphics_deferred_pipeline
            = builder.BuildGraphicsPipeline(device.Get(), L"Deferred Graphics Pipeline").value();
...
void DeferredRenderer::RenderFrame(const Camera& camera, DXDevice& device, DXSwapchain& swapchain_target)
{
    ...
    //     CD3DX12_VIEWPORT viewport = CD3DX12_VIEWPORT(0, 0.0f, 0.0f, resolution.x, resolution.y);
    ...
    //     std::array<CD3DX12_CPU_DESCRIPTOR_HANDLE, 4> rtv_handles = {
    //         render_target_heap.GetCPUAddress(0).value(),
    //         render_target_heap.GetCPUAddress(1).value(),
    //         render_target_heap.GetCPUAddress(2).value(),
    //         render_target_heap.GetCPUAddress(3).value()
    //     };

    //     CD3DX12_CPU_DESCRIPTOR_HANDLE depth_descriptor = depth_heap.GetCPUAddress(0).value();

    //     command_list.Get()->OMSetRenderTargets(4, rtv_handles.data(), false, &depth_descriptor);
    ...
    {
        LightInfo data {};
        data.mAmbientAndIntensity = { 1.0f, 1.0f, 1.0f, 1000.0f };
    ...
    // For each model, draw in GBuffers
    {
        for (const auto& [mat, mesh] : models_to_render)
        {
            ...
            {
                MaterialInfo data {};
                data.colorFactor = { 1.0f, 1.0f, 1.0f, 1.0f };

                detail::WriteResource(material_info_data, data);
                command_list.BindRootCBV(material_info_data, shader_inputs.GetInputIndex("material_info").value());
            }
            // Set Input Assembly
            {
