...
ComPtr<ID3D12RootSignature> DXSignatureBuilder::Build(ComPtr<ID3D12Device5> device, LPCWSTR name) const
{
    ...
    if (FAILED(hr))
    {
        Log("{}", (const char*)errBlob->GetBufferPointer());
...
DXSignatureBuilder& DXSignatureBuilder::Add32BitConstant(const uint32_t shaderRegister, D3D12_SHADER_VISIBILITY shader, int num32BitValues)
{
...
DXSignatureBuilder& DXSignatureBuilder::AddCBuffer(const uint32_t shaderRegister, D3D12_SHADER_VISIBILITY shader)
{
    D3D12_ROOT_DESCRIPTOR desc;
    desc.RegisterSpace = 0;
    desc.ShaderRegister = shaderRegister;
...
DXSignatureBuilder& DXSignatureBuilder::AddSampler(const uint32_t shaderRegister, D3D12_SHADER_VISIBILITY shader, D3D12_TEXTURE_ADDRESS_MODE mode, D3D12_FILTER filter, D3D12_STATIC_BORDER_COLOR color, D3D12_COMPARISON_FUNC comparison)
{
