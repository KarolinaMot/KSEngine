...
Shader::Shader(const Device& device, ShaderType shaderType, std::shared_ptr<ShaderInputs> shaderInput, std::string path, Formats* rtFormats, int numFormats)
{
    ...
    if (shaderType == ShaderType::ST_MESH_RENDER)
    {
        ...
        if (!rtFormats)
        {
            Log("Render target format has not been passed. A default format of R8G8B8A8_UNORM will be added instead");
...
Shader::Shader(const Device& device, ShaderType shaderType, void* shaderInput, std::string path)
{
    // m_shader_input = shaderInput;
    m_shader_type = shaderType;
    m_impl = std::make_unique<Impl>();
