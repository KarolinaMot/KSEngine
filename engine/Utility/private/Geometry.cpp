...
BoundingBox::BoundingBox(const glm::vec3& m_center, const glm::vec3& m_extents)
    : m_center(m_center)
    , m_extents(glm::vec3(std::fabsf(m_extents.x), std::fabsf(m_extents.y), std::fabsf(m_extents.z)))
{
}

std::array<glm::vec3, 8> BoundingBox::GetEdgePoints() const
{
    return {
...
BoundingBox BoundingBox::ApplyTransform(const glm::mat4& transform) const
{
...
bool BoundingBox::FrustumTest(const std::array<Plane, 6>& frustum) const
{
...
Plane::Plane(const glm::vec3& normal, float m_signedOriginDistance)
    : m_normal(normal)
...
Camera Camera::Perspective(const glm::vec3& position, const glm::vec3 lookAt, float aspectRatio, float fieldOfView, float nearClip, float farClip)
{
    Camera camera;
...
Camera Camera::Orthographic(const glm::vec3& position, const glm::vec3 lookAt, float aspectRatio, float extents, float nearClip, float farClip)
{
    Camera camera;
