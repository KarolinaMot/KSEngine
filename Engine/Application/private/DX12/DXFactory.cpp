...
std::optional<ComPtr<ID3D12Device>> DXFactory::CreateDevice(DXGI_GPU_PREFERENCE preference, std::function<bool(CD3DX12FeatureSupport)> feature_eval)
{
    ...
    Log("Failed to create DX12 Device, crash is emminent");
    return std::nullopt;
}
