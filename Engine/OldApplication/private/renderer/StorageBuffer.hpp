...
public:
    ...
    void Update(const Device& device, const std::vector<T>& data)
    {
        ...
        {
            Log("StorageBuffer {} type on update does not fit the original format. Command has been ignored.", m_name);
            return;
        }
    ...
    void Update(const Device& device, const T* data, size_t numElements)
    {
        ...
        if (data == nullptr)
        {
            Log("StorageBuffer {} got an invalid data value for update. Command ignored.", m_name);
            return;
        }
