...
namespace KS
{
    ...
    public:
        ...
        Buffer(const Device &device, const std::string &name, const std::vector<T> &data, bool readWriteEnabled)
        {
            ...
            m_total_buffer_size = sizeof(T) * data.size();
            m_num_elements = data.size();
            m_name = name;
            CreateStructuredBuffer(device, name, sizeof(T), m_num_elements);
            UploadDataStructuredBuffer(device, data.data(), m_num_elements);
        }
        ...
        template <typename T>
        Buffer(const Device& device, const std::string& name, uint32_t width, uint32_t height, const std::vector<T>& data, bool readWriteEnabled)
        {
            m_name = name;
        }
