...
//    std::shared_ptr<DXResource> m_uploadBuffers[FRAME_BUFFER_COUNT];
...
//KS::TLAS::~TLAS() {}
//
//void KS::TLAS::AddInstance(const Device& device, DXCommandList& cmd, Mesh* mesh, glm::mat4x4 modelMat)
//{
...
//    m_instances[instanceHandle].m_mesh->SetTLASHandle(instanceHandle);
//    m_Impl->m_updateStructure[0] = true;
//    m_Impl->m_updateStructure[1] = true;
//}
//
...
//       m_Impl->m_uploadBuffers[i] = std::make_shared<DXResource>(engineDevice, uploadHeapProperties, resourceDesc, nullptr,
//                                                                "TLAS default upload buffer", D3D12_RESOURCE_STATE_GENERIC_READ);
//
//       HRESULT hr = m_Impl->m_uploadBuffers[i]->GetResource()->Map(0, nullptr, reinterpret_cast<void**>(&m_Impl->m_CPU[i]));
//       if (FAILED(hr))
//       {
...
//}
//
//void KS::TLAS::WriteInstanceDescs(uint32_t frameIndex)
//{
//    const UINT count = static_cast<UINT>(m_instances.size());
//
//    auto descs = reinterpret_cast<D3D12_RAYTRACING_INSTANCE_DESC*>(m_Impl->m_CPU[frameIndex]);
//
...
//        d.AccelerationStructure = s.m_mesh->BLASAddress();
//        descs[i] = d;
//    }
//}
//
...
//
//    cmd.GetCommandList()->CopyBufferRegion(dst->GetResource().Get(), 0, dst->GetUploadResource(0).Get(), 0,
//                                           bytes);
//
