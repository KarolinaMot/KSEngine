...
public:
    static std::shared_ptr<DXDescHeap> Construct(ComPtr<ID3D12Device5> device, int numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, LPCWSTR name, int heapStart =0, D3D12_DESCRIPTOR_HEAP_FLAGS flags = D3D12_DESCRIPTOR_HEAP_FLAGS::D3D12_DESCRIPTOR_HEAP_FLAG_NONE)
    {
        auto variable = std::make_shared<DXDescHeap>(device, numDescriptors, type, name, flags, heapStart);
        return variable;
    }
    ...
    DXDescHeap(ComPtr<ID3D12Device5> device, int numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, LPCWSTR name, D3D12_DESCRIPTOR_HEAP_FLAGS flags, int heapStart);
