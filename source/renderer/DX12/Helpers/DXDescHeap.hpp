...
public:
    static std::shared_ptr<DXDescHeap> Construct(ComPtr<ID3D12Device5> device, uint32_t numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, LPCWSTR name, D3D12_DESCRIPTOR_HEAP_FLAGS flags = D3D12_DESCRIPTOR_HEAP_FLAGS::D3D12_DESCRIPTOR_HEAP_FLAG_NONE)
    {
    ...
    DXDescHeap(ComPtr<ID3D12Device5> device, uint32_t numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, LPCWSTR name,
...
private:
    ...
    uint32_t mResourceCount = OTHER_RESOURCES_START;
